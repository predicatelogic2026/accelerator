...
int main(int argc, char *argv[])
{
    ...
    *nc = NUM_CONSTANTS; 
    ...
    for (int pred = 1; pred <= lim; pred++)
	    ...
	    //Tuple tt = (Tuple){{(uint16_t)pred, (uint16_t)consts[c], 0, 0}};
	    //printf("------- 1 init fact %lx, arity %d\n", *((uint64_t *)&tt), 2);
	}
    ...
    for (int pred = FULL_COVERAGE_FIRST_N + 1; pred <= NUM_UNARY; pred++) {
	    ...
	    //Tuple tt = (Tuple){{(uint16_t)pred, (uint16_t)sel[i], 0, 0}};
	    //printf("------- 2 init fact %lx, arity %d\n", *((uint64_t*)&tt), 2);
    ...
    if (NUM_HUB_CONSTANTS > 0) {
        ...
        for (int pred = 1; pred <= NUM_UNARY; pred++)
	        ...
	        //Tuple tt = (Tuple){{(uint16_t)pred, (uint16_t)hubs[i], 0, 0}};
	        //printf("------- 3 init fact %lx, arity %d\n", *((uint64_t *)&tt), 2);
	    }
        free(hubs);
    }
    ...
    for (int i = 0; i < *nc; i++)
        for (int j = i + 1; j < *nc; j++){
            ...
            //Tuple tt = (Tuple){{0, (uint16_t)consts[i], (uint16_t)consts[j], 0}};
	    //printf("------- 4 init fact %lx, arity %d\n", *((uint64_t *)&tt), 3);
	}
	...
	uint64_t *pt = (uint64_t *)prems;
	if(i==0) printf("?????? %lx, %lx, %lx", pt[0], pt[1], pt[2]);
